...
Built a full forecasting system for a live, scored World Cup prediction competition. The central finding: the platform's crowd consensus is biased toward uncertainty — it holds back from confident predictions even when the evidence clearly supports one — and that bias was stable even after the number of predictions used to measure it doubled. I also tested, rigorously and more than once, whether a machine-learning model could improve on a hand-built statistical approach. It couldn't, at this amount of data — a tested conclusion, not an assumption.
